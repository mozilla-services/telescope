    ...
    mock_aioresponses.get(
        ...
        payload={
            "changes": [
                ...
                {"id": "efg", "attachment": {"location": "missing.jpg", "size": 7}},
                {
                    "id": "ijk",
...
async def test_urls_slicing(
    ...
    mock_aioresponses.get(
        ...
        payload={
            "changes": [
                {"id": f"id{i}", "attachment": {"location": f"file{i}.jpg", "size": 10}}
                for i in range(100)
            ]
        },
    )
...
async def test_urls_slicing_by_size(mock_aioresponses):
    server_url = "http://fake.local/v1"
    mock_aioresponses.get(
        server_url + "/",
        payload={"capabilities": {"attachments": {"base_url": "http://cdn/"}}},
    ...
    mock_aioresponses.get(
        ...
        payload={
            "changes": [
                {"id": "id-0", "attachment": {"location": "file-big.jpg", "size": 100}},
                {
                    "id": "id-1",
                    "attachment": {"location": "file-small1.jpg", "size": 10},
                },
                {
                    "id": "id-2",
                    "attachment": {"location": "file-small2.jpg", "size": 10},
                },
                {
                    "id": "id-3",
                    "attachment": {"location": "file-small3.jpg", "size": 10},
                },
            ]
        },
    )
    ...
    with mock.patch(
        ...
        mocked.return_value = {}, True
        await run(server_url, slice_percent=(0, 50))
    calls = mocked.call_args_list
    # Only the big file should be in the first 50% slice.
    assert len(calls) == 1
    assert calls[0][0][0]["location"] == "http://cdn/file-big.jpg"
